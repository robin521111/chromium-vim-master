...
var matchLocation = function(url, pattern) { // Uses @match syntax
  ...
  if (!protocol.length) {
    console.error('rVim Error: Invalid protocol in pattern: "%s"', pattern);
    return false;
  }
  ...
  if (url.protocol !== 'file:') {
    ...
    if (!hostname) {
      console.error('rVim Error: Invalid host in pattern: "%s"', pattern);
      return false;
    }
...
var waitForLoad = function(callback, constructor) {
  ...
  (typeof window !== 'undefined' ? window : self).setTimeout(function() {
...
// parseConfig function - works in both content script and service worker environments
...
// Expose parseConfig to window object if available (content script environment)
