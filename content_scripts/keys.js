...
if (HAS_EVENT_KEY_SUPPORT) {
  ...
  KeyListener = (function() {
    ...
    var KeyEvents = {
      ...
      keyup: function(event) {
        ...
        if (Hints.active && event.which === 191) {
          setTimeout(function() {
            document.getElementById('rVim-link-container').style.opacity = '1';
          }, 0);
        }
      ...
      keydown: function(callback, event) {
        ...
        // Otherwise, non-rVim keyboard listeners will not be stopped.
    ...
    var parseLangMap = (function() {
      ...
      function parseError(error) {
        throw Error('rVim langmap error: ' + error);
      }
      function parseObj(tokens, pairs) {
        var len = tokens.length;
        var mid = len >> 1;
...
var KeyHandler = {
  down: function(key, event) {
    ...
    if (Hints.active) {
      ...
      case 191: // Slash
        event.preventDefault();
        document.getElementById('rVim-link-container').style.opacity = '0';
        return;
      }
    }
