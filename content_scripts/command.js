...
Command.setupFrameElements = function() {
  ...
  this.bar.id = 'rVim-command-bar';
  this.bar.rVim = true;
  this.bar.style[(this.onBottom) ? 'bottom' : 'top'] = '0';
  this.modeIdentifier = document.createElement('span');
  this.modeIdentifier.id = 'rVim-command-bar-mode';
  this.modeIdentifier.rVim = true;
...
Command.setup = function() {
  ...
  this.input.id = 'rVim-command-bar-input';
  this.input.rVim = true;
...
Command.commandBarFocused = function() {
  return commandMode && this.active && document.activeElement &&
    document.activeElement.id === 'rVim-command-bar-input';
};
...
Command.updateCompletions = function(useStyles) {
  ...
  for (i = 0; i < this.completionResults.length; ++i) {
    ...
    item.className = 'rVim-completion-item';
    var identifier;
    if (useStyles &&
    ...
    if (this.completionResults[i].length >= 3) {
      ...
      left.className = 'rVim-left';
      left.textContent = this.completionResults[i][1];
      var right = document.createElement('span');
      right.className = 'rVim-right';
...
Command.execute = function(value, repeats) {
  ...
  // = == force rVim to treat text as a URL
  // ? == force rVim to tread text as a search
  ...
  if (/^source/.test(value)) {
    ...
    RUNTIME('loadLocalConfig', { path: path }, function(res) {
      if (res.code === -1) {
        ...
        console.error('rvim error: "%s" could not be opened for parsing', path);
      }
    });
    return;
  }
...
Command.insertCSS = function() {
  ...
  if (settings.linkanimations) {
    css += '.rVim-link-hint { transition: opacity 0.2s ease-out, ' +
...
Command.onDOMLoad = function() {
  ...
  if (window.self === window.top) {
    ...
    Command.frame.id = 'rVim-command-frame';
...
Command.init = function(enabled) {
  ...
  if (enabled) {
    ...
    var links = document.getElementById('rVim-link-container');
