...
Hints.hideHints = function(reset, multi, useKeyDelay) {
  if (reset && document.getElementById('rVim-link-container') !== null) {
    document.getElementById('rVim-link-container')
      .parentNode.removeChild(document.getElementById('rVim-link-container'));
  } else if (document.getElementById('rVim-link-container') !== null) {
    if (!multi)
      HUD.hide();
    if (settings.linkanimations) {
      Hints.shadowDOM.addEventListener('transitionend', function() {
        var m = document.getElementById('rVim-link-container');
        if (m !== null) {
...
Hints.removeContainer = function() {
  var hintContainer = document.getElementById('rVim-link-container');
...
Hints.handleHint = function(key) {
  ...
  switch (key) {
  case '/':
    return document.getElementById('rVim-link-container').style.opacity = '0';
  case '<Tab>':
...
Hints.create = function(type, multi) {
  ...
  window.setTimeout(function() {
    ...
    Hints.linkElementBase.rVim = true;
  Hints.linkElementBase.className = 'rVim-link-hint';
    if (settings.scalehints) {
      Hints.linkElementBase.className += ' rVim-hint-scale';
    }
    ...
    [].forEach.call(document.querySelectorAll('style'), function(e) {
      if (e.textContent.indexOf('rVim') !== -1) {
