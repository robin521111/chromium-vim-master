 ...
 * What features does rVim add to Chrome?
 ...
 * Boolean rVimrc settings are enabled with the command ```'set' + <SETTING_NAME>``` and disabled with
   the command ```'set' + no<SETTING_NAME>``` (for example, ```set regexp``` and ```set noregexp```)
 * Boolean rVimrc settings can be inversed by adding "!" to the end
