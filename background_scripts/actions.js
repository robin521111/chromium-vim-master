...
Actions = (function() {
  ...
  _.portCallback = (function() {
    ...
    var retval = function(o) {
      if (typeof callbacks[o.request.id] === 'function') {
  ...
  _.injectCSS = function(o) {
    chrome.scripting.insertCSS({
      ...
      // Handle errors silently - prevent exceptions when injecting
      // CSS into unsupported URLs (chrome://*, etc)
    });
  };
  ...
  _.loadLocalConfig = function(o) {
    ...
    httpRequest({ url: path }).then(function(data) {
      var added = self.parseConfig(data);
      if (added.error) {
        console.error('parse error on line %d of rVimrc: %s',
  ...
  var actionHandler = function(_request, _sender, _callback, _port) {
